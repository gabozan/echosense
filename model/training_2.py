...
def create_advanced_model(input_shape, num_classes):
    ...
    # Bloque 1: Filtros pequeños para captar texturas básicas
    x = tf.keras.layers.Conv2D(16, (3, 3), padding='same', activation='relu')(inputs)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)
    
    # Bloque 2: Filtros intermedios
    x = tf.keras.layers.Conv2D(32, (3, 3), padding='same', activation='relu')(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.MaxPooling2D((2, 2))(x)
    
    # Bloque 3: Reducción drástica para ahorrar RAM
    x = tf.keras.layers.Conv2D(64, (3, 3), padding='same', activation='relu')(x)
    x = tf.keras.layers.BatchNormalization()(x)
    
    # GlobalAveragePooling es MAGIA: sustituye a capas Dense gigantes
    # y reduce el peso del modelo un 70% sin perder mucha precisión.
    x = tf.keras.layers.GlobalAveragePooling2D()(x)
    
    # Clasificador final ligero
